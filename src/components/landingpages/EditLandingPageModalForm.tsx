...
>((props, ref) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.redirectTo.trim()) {
      newErrors.redirectTo = "Redirect URL is required.";
    }
    if (!formData.redirectTo.trim()) {
      newErrors.redirectTo = "Redirect URL is required.";
    }
    ...
    <div className="space-y-6 p-4 md:p-6 bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border dark:border-gray-700">
        ...
        <div className={`grid grid-cols-1 gap-3 
            ${userRoleId === 1 ? 'sm:grid-cols-3' : 'sm:grid-cols-2'}`}>
          <div>
            <LabelWithTooltip required>Name</LabelWithTooltip>
            <Input
              id="page-name"
              ...
              className={`w-full text-sm sm:text-base h-10 px-3 ${errors.name ? 'border-1 border-red-500 dark:border-1 dark:border-red-500': ''}`}
          ...
          <div>
            <LabelWithTooltip required tooltip="The URL to redirect users after they submit the form">Redirect URL</LabelWithTooltip>
            <Input
              id="redirect-url"
              ...
              className={`w-full text-sm sm:text-base h-10 px-3 ${errors.redirectTo ? 'border-1 border-red-500 dark:border-1 dark:border-red-500': ''}`}
              disabled={isSubmitting}
              required
            />
            {errors.redirectTo && <p className="text-red-500 text-xs mt-1">{errors.redirectTo}</p>}
          </div>
